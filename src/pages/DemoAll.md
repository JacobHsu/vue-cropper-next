...
這個頁面把常用的 props、旋轉方法、導出結果統一放在一起，方便做「配置驅動」的驗證。
...
<script setup>
  ...
  const labels = computed(() => isEn.value ? {
    ...
    wrapperWidth: '容器寬度',
    wrapperHeight: '容器高度',
    cropLayoutWidth: '截圖框寬度',
    cropLayoutHeight: '截圖框高度',
    export: '導出',
    outputType: '輸出格式',
    outputSize: '輸出品質',
    full: '高分屏導出',
  ...
  const filterOptions = computed(() => isEn.value ? [
    ...
    { label: '無濾鏡', value: 'none' },
    { label: '灰度濾鏡', value: 'grayscale' },
    { label: '黑白濾鏡', value: 'bw' },
    { label: '老照片濾鏡', value: 'old' },
  ])
