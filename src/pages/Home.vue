...
const t = computed(() => {
  ...
  return {
    title: 'cropper-next-vue',
    subtitle: '一個獨立發佈的 Vue 3 圖片裁剪庫，重點處理旋轉邊界判斷、邊界限制、高分屏導出與即時預覽。',
    primaryCta: '開始使用',
    docsCta: '查看文件',
    githubCta: 'GitHub',
    npmCta: 'npm',
    ...
    features: [
      {
        title: '旋轉後的邊界判斷',
        desc: '圖片旋轉後仍能穩定判斷邊界與限制邏輯。'
      },
      {
        title: '限制在截圖框/容器內',
        desc: '支援將圖片限制在截圖框或容器內，避免拖出可視範圍。'
      },
      {
        title: '高分屏導出',
        desc: '提升導出品質，適配高 DPI 螢幕。'
      },
      {
        title: '即時預覽',
        desc: '透過事件即時取得預覽資料，便於業務聯動。'
      },
      {
        title: '獨立構建',
        desc: 'npm 套件與文件站可分別構建發佈。'
      }
    ],
